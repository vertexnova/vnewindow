...
class XwinEventsExample final : public vne::xwin::examples::ExampleBase {
   public:
    ...
    void onInit(vne::xwin::IWindow& /*window*/,
        ...
        hooks.on_key_down = [](vne::xwin::IWindow* win,
                               vne::events::KeyCode   key,
                               std::uint8_t           mods,
                               bool                   repeat) {
            VNE_LOG_INFO << "key_down  win=" << win
                         << " key=" << static_cast<int>(key)
                         << " mods=" << static_cast<int>(mods)
                         << " repeat=" << repeat;
        };

        hooks.on_key_up = [](vne::xwin::IWindow* win,
                             vne::events::KeyCode key,
                             std::uint8_t         mods) {
            VNE_LOG_INFO << "key_up    win=" << win
                         << " key=" << static_cast<int>(key)
                         << " mods=" << static_cast<int>(mods);
        };

        hooks.on_mouse_move = [](vne::xwin::IWindow* win, double x, double y,
                                 std::uint8_t /*mods*/) {
            (void)win; (void)x; (void)y; // uncomment to enable verbose mouse-move logs
        };

        hooks.on_mouse_button = [](vne::xwin::IWindow* win,
                                   vne::events::MouseButton btn, bool pressed,
                                   double x, double y, std::uint8_t /*mods*/) {
            VNE_LOG_INFO << "mouse_btn win=" << win
                         << " btn=" << static_cast<int>(btn)
                         << " pressed=" << pressed
                         << " pos=(" << x << "," << y << ")";
        };

        hooks.on_touch = [](vne::xwin::IWindow* /*win*/, std::uint32_t touch_id,
                            double x, double y,
                            vne::xwin::EventBridgeTouchPhase phase) {
            VNE_LOG_INFO << "touch id=" << touch_id
