...
const xdg_wm_base_listener g_xdg_wm_base_listener = [] {
...
const wl_touch_listener g_touch_listener = [] {
    wl_touch_listener l{};
    l.down = touchDown;
    l.up = touchUp;
...
void WaylandWindowManager::onSeatCapabilities(struct wl_seat* seat, uint32_t caps) {
    ...
    if (has_kb && !keyboard_) {
        ...
        if (keyboard_) {
            wl_keyboard_add_listener(keyboard_, &g_keyboard_listener, this);
        }
    ...
    if (has_ptr && !pointer_) {
        ...
        if (pointer_) {
            wl_pointer_add_listener(pointer_, &g_pointer_listener, this);
        }
    ...
    if (has_tch && !wl_touch_) {
        ...
        if (wl_touch_) {
            wl_touch_add_listener(wl_touch_, &g_touch_listener, this);
        }
...
void WaylandWindowManager::onRegistryGlobal(struct wl_registry* registry,
    ...
    if (std::strcmp(interface, wl_compositor_interface.name) == 0) {
        ...
        if (output) {
            wl_output_add_listener(output, &g_output_listener, this);
...
void WaylandWindowManager::bindXdgWmBase(struct wl_registry* registry, uint32_t name, uint32_t version) {
    ...
    if (xdg_wm_base_) {
        xdg_wm_base_add_listener(xdg_wm_base_, &g_xdg_wm_base_listener, nullptr);
    }
}
...
void WaylandWindowManager::bindSeat(struct wl_registry* registry, uint32_t name, uint32_t version) {
    ...
    if (seat_) {
        wl_seat_add_listener(seat_, &g_seat_listener, this);
    }
}
...
bool WaylandWindowManager::initialize() {
    ...
    wl_registry_add_listener(registry_, &g_registry_listener, this);
